...
/**
 * Test visuel pour vérifier l'alignement et l'apparence des maquettes
 */
test.describe('Vérification visuelle des maquettes', () => {
  test('PrepareriXFacture - Vérification de l\'alignement', async ({ page }) => {
    // Naviguer vers la maquette
    await page.goto('/preparer-ixfacture');
    ...
    await page.screenshot({
      path: 'tests/screenshots/preparer-ixfacture-full.png',
      fullPage: true
    });
    ...
    await toolbar.screenshot({
      path: 'tests/screenshots/preparer-ixfacture-toolbar.png'
    });
    ...
    if (appBarBox && toolbarBox && tableBox) {
      ...
      // L'AppBar a généralement px: 3 (24px) sur les côtés
